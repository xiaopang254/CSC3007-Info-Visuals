...
5. Based on previous visualisation, readers might interpret that covid-19 was an attribute of the traffic accident which might be misleading since the data is solely discussing about traffic accidents resulting in injuries and have nothing related to the pandemic.
...
## Data Manipulation
* The original dataset included many types of accident causes. To simplify the visualization, we categorized similar causes together and renamed them with a more general name.
* When exporting the dataset from the website, the CSV file displayed ‘NA’ for cells that have a value of 0. Hence, we need to manipulate the cells to display 0 to reflect the original dataset
* The original dataset’s columns were not consistent in terms of data class types. The cells under columns 2017 to 2022 have the ‘dbl’ data class whereas cells under columns 2012 to 2016 have the ‘chr’ data class. To allow for data combination, we had to mutate everything from row 1 column 2 into ‘dbl’ data class.
...
* *readr* for data import. *tidyverse* for data transformation, including *ggplot2* for visualization based on the grammar of graphics. *knitr* for dynamic document generation
* *gt*, *gtExtras*, *grid*, *gridExtra*, *sparklines* for creating sparkline table
* *patchwork* for combining overview graphs side-by-side
* *png* to create the chart into images
